...
pub async fn git_stash_save(
    ...
    let out = git::run_ok(&repo, &["stash", "push", "-u", "-m", &message]).await?;
...
pub async fn git_stash_pop(
    ...
    git::run_ok(&repo, &["stash", "pop", &stash_ref]).await?;
    Ok(())
}
...
mod tests {
    ...
    #[tokio::test]
    async fn stash_save_returns_ref_when_dirty() {
        let dir = dirty_repo("dirty");
        let r = git_stash_save(dir.to_string_lossy().into_owned(), "wip".into()).await.unwrap();
        assert_eq!(r, "stash@{0}");
    }

    #[tokio::test]
    async fn stash_save_returns_empty_when_clean() {
        ...
        let r = git_stash_save(dir.to_string_lossy().into_owned(), "noop".into()).await.unwrap();
        assert_eq!(r, "");
    }

    #[tokio::test]
    async fn stash_pop_restores_work() {
        let dir = dirty_repo("pop");
        let stash_ref = git_stash_save(dir.to_string_lossy().into_owned(), "wip".into()).await.unwrap();
        assert!(!stash_ref.is_empty());
        git_stash_pop(dir.to_string_lossy().into_owned(), stash_ref).await.unwrap();
        assert!(dir.join("wip.txt").exists());
    }

    #[tokio::test]
    async fn stash_pop_empty_ref_is_noop() {
        let dir = dirty_repo("noop");
        // No prior save — empty ref must not error.
        git_stash_pop(dir.to_string_lossy().into_owned(), "".into()).await.unwrap();
    }
}
